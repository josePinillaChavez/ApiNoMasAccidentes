...
namespace ApiNoMasAccidentes.Models
{
	public class EmpresaModel
	{
		...
		public string ListarEmpresa()
		{
			...
			List<EmpresaResponse> empresalist = new List<EmpresaResponse>();
			...
			try
			{
				using (OracleConnection conn = new OracleConnection("DATA SOURCE=localhost:1521;PASSWORD=DUOCNMA;USER ID=DUOCNMA;"))
				{
					...
					foreach (DataRow row in dt.Rows)
					{
						...
						empresa.dv_rut = row["dv_rut"].ToString();
						empresa.nombre = row["nombre"].ToString();
						if (int.Parse(row["id_empresa"].ToString()) == 21)
						{

						}
						empresa.telefono = int.Parse(row["telefono"].ToString());
						empresa.email = row["email"].ToString();
						empresa.vigente = row["vigente"].ToString();
						empresa.descripcion = row["descripcion"].ToString();
						empresalist.Add(empresa);
					}

					JSONString = JsonConvert.SerializeObject(empresalist);
